...
export function isCurrentPlayer(connectionId: ConnectionId, lobby: Lobby): boolean {
    return connectionId === lobby.players[lobby.gameState.Player]
}

export function playerJoin(connectionId: string, lobbyState: Lobby): Lobby {
    ...
    if (!newLobby.players["x"]) {
        ...
        console.log("Either add the ability to join as a spectator or log an error; for now, we won't update lobby state.")
    }
